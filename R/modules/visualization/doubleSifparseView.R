...
setNodosFunctionVis <- function(sifx, numSif){
  ...
  for(i in c(1:length(interactions$INTERACTION_TYPE))){
    ...
    switch(interactions$INTERACTION_TYPE[i],
           ...
           "controls-phosphorylation-of" = {
             controlPhospho<-paste("phospho_control", counterPhospho, numSif)
           ...
           "controls-state-change-of" = {
             controlState <- paste("controlState",counterControlState, numSif)
             source <- c(source, ParticipantB)
           ...
           "controls-expression-of" = {
             ...
             controlExpresion <- paste("control_expression", counterExpresion, numSif)
             source <- c(source, ParticipantA)
           ...
           ,
           "chemical-affects" ={
             controlChemicalAffects <- paste("control_chemical", counterChemical, numSif)
             source <- c(source, ParticipantA)
  ...
  if(length(protein.catalysis)>=1){
    for (i in c(1:length(protein.catalysis))){
      ...
      for(j in c(1:length(imput.sm))){
        control <- paste("control", counterCatalysis, numSif)
        if(impt){
...
parseDoubleSifVis <- function(sifx1,sifx2){
  nodesSif1 <- setNodosFunctionVis(sifx1, 1)
  nodesSif2 <- setNodosFunctionVis(sifx2, 2)
