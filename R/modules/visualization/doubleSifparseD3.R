...
setNodosFunction <- function(sifx, numberSif){
  ...
  for(i in c(1:length(interactions$INTERACTION_TYPE))){
    ...
    switch(interactions$INTERACTION_TYPE[i],
           ...
           "controls-phosphorylation-of" = {
             controlPhospho<-paste("phospho_control", counterPhospho, numberSif)
           ...
           "controls-state-change-of" = {
             controlState <- paste("controlState",counterControlState, numberSif)
             source <- c(source, ParticipantB)
           ...
           "controls-expression-of" = {
             ...
             controlExpresion <- paste("control_expression", counterExpresion, numberSif)
             source <- c(source, ParticipantA)
           ...
           ,
           "chemical-affects" ={
             controlChemicalAffects <- paste("control_chemical", counterChemical, numberSif)
             source <- c(source, ParticipantA)
  ...
  if(length(protein.catalysis)>=1){
    for (i in c(1:length(protein.catalysis))){
      ...
      for(j in c(1:length(imput.sm))){
        control <- paste("control", counterCatalysis, numberSif)
        if(impt){
  ...
  return(list(nodos, tipoNodo, Links))
}

parseDoubleSifD3 <- function(sifx1,sifx2){
  nodesSif1 <- setNodosFunction(sifx1, 1)
  nodesSif2 <- setNodosFunction(sifx2, 2)

  nodos <- c()
  tipoNodo <- c()

  for(n in nodesSif1[[1]]){
    pos <- grep(n, nodesSif2[[1]])
    if(length(pos)){
      if(nodesSif2[[2]][pos] == "NProt"){
        nodesSif2[[2]][pos] <- "NProtC"
      }

      if(nodesSif2[[2]][pos] == "NSM"){
        nodesSif2[[2]][pos] <- "NSMC"
      }
    }
  }
  nodos <- nodesSif2[[1]]
  tipoNodo <- nodesSif2[[2]]
  for(n in nodos){
    pos <- grep(n, nodesSif1[[1]])
    if(length(pos)){
      nodesSif1[[1]] <- nodesSif1[[1]][-pos]
      nodesSif1[[2]] <- nodesSif1[[2]][-pos]
    }
  }

  nodos <- c(nodos, nodesSif1[[1]])
  tipoNodo <- c(tipoNodo, nodesSif1[[2]])
  FinalNodes <- data.frame(nodos, tipoNodo)
  Links <- rbind(nodesSif1[[3]], nodesSif2[[3]])
